...
class MeshtasticBridge:
    def __init__(self, config: BridgeConfig) -> None:
        ...
        self._node_paths: Optional[NodePaths] = None
        self._packet_listener = self._on_packet

    def _configure_logging(self) -> None:
        self.config.logs_dir.mkdir(parents=True, exist_ok=True)
        formatter = logging.Formatter("%(asctime)s %(levelname)s %(message)s")
        file_handler = logging.FileHandler(self.config.logs_dir / "meshtastic-bridge.log", encoding="utf-8")
    ...
    def _register_listeners(self) -> None:
        assert self._interface is not None
        self._interface.addPacketListener(self._packet_listener)

    def _remove_listeners(self) -> None:
        if not self._interface:
            return
        remove = getattr(self._interface, "removePacketListener", None)
        if callable(remove):
            remove(self._packet_listener)
    ...
    def _on_packet(self, packet: Dict[str, Any]) -> None:
        try:
            ...
            if portnum == portnums.PORTNUM_TELEMETRY_APP:
                self._handle_telemetry(packet)
            elif portnum in (portnums.PORTNUM_TEXT_MESSAGE_APP, portnums.PORTNUM_PRIVATE_APP):
